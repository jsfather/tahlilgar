...
const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <ThemeContext.Provider
      value={{ isDark, toggleTheme, primaryColor, setPrimaryColor }}
    >
      <div
...
const Navbar = ({ settings }: { settings: SiteSettings }) => {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 border-b transition-all duration-300 ${isDark ? "bg-slate-900/80 border-slate-800" : "bg-white/80 border-gray-100"} backdrop-blur-md`}
    >
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="text-right">
            ...
            >
              <a
                href="#forms"
                className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-white rounded-2xl font-bold text-lg hover:opacity-90 hover:scale-105 transition-all shadow-xl shadow-primary/20"
              >
                شروع تحلیل دارایی
  ...
  return (
    <section className="py-12 max-w-4xl mx-auto px-4 text-center">
      ...
      >
        ...
        <p
          className={`text-2xl font-medium leading-relaxed italic ${isDark ? "text-slate-300" : "text-primary-900"}`}
        >
          {quote}
        </p>
      </motion.div>
...
const PublicForm = ({
      ...
      >
        ...
        <div className="aspect-video bg-slate-900 rounded-3xl overflow-hidden mb-8 shadow-xl">
          {settings.success_video_url ? (
            <video
              src={settings.success_video_url}
              controls
              autoPlay
              className="w-full h-full object-cover"
            />
  ...
  return (
    <div className="relative">
      ...
      >
        <h3
          className={`text-3xl font-bold mb-10 text-center ${isDark ? "text-white" : "text-gray-900"}`}
        >
          {form.name}
        </h3>
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid md:grid-cols-2 gap-8">
            {form.fields?.map((field) => (
              ...
              >
                <label
                  className={`block text-sm font-bold pr-2 ${isDark ? "text-slate-400" : "text-gray-700"}`}
                >
                  {field.label}{" "}
...
const HomePage = () => {
  ...
  return (
    <>
      ...
      <footer
        className={`border-t py-12 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-100"}`}
      >
...
const AdminPanel = () => {
          ...
          .then((data) => {
            ...
            setSubmissions(
              data.map((s: any) => ({
                ...s,
                is_duplicate:
                  s.email && s.email.trim() !== "" && counts[s.email] > 1,
              })),
            );
          });
        fetch("/api/forms")
  ...
  return (
    <div
      className={`min-h-screen flex ${isDark ? "bg-slate-900 text-white" : "bg-gray-50 text-gray-900"}`}
      dir="rtl"
    >
      {/* Sidebar */}
      <aside
        className={`w-72 border-l flex flex-col sticky top-0 h-screen ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-100"}`}
      >
        <div
          className={`p-8 border-b ${isDark ? "border-slate-700" : "border-gray-100"}`}
        >
          <h1
          ...
          <div className="mt-4 flex gap-4">
            <div
              className={`flex-1 p-3 rounded-2xl ${isDark ? "bg-blue-900/30" : "bg-blue-50"}`}
            >
              <span
                className={`text-xs block ${isDark ? "text-blue-300" : "text-blue-400"}`}
              >
                آنلاین
              </span>
              <span
            ...
            <div
              className={`flex-1 p-3 rounded-2xl ${isDark ? "bg-indigo-900/30" : "bg-indigo-50"}`}
            >
              <span
                className={`text-xs block ${isDark ? "text-indigo-300" : "text-indigo-400"}`}
              >
                کل بازدید
              </span>
              <span
        ...
        <div
          className={`p-6 border-t ${isDark ? "border-slate-700" : "border-gray-100"}`}
        >
          <Link
            to="/"
            className={`flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${isDark ? "text-slate-400 hover:bg-slate-700" : "text-gray-600 hover:bg-gray-50"}`}
          >
            <Home size={22} />
            مشاهده سایت
          </Link>
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="space-y-8">
              ...
              <div className="grid gap-6">
                {filteredLeads?.map((sub) => (
                  <div
                    key={sub.id}
                    className={`p-8 rounded-[2rem] border transition-all ${sub.is_duplicate ? (isDark ? "border-amber-900 bg-amber-900/10" : "border-amber-200 bg-amber-50/30") : isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-100 shadow-sm"}`}
                  >
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isDark ? "bg-blue-900/30 text-blue-400" : "bg-blue-100 text-blue-600"}`}
                        >
                          <Users size={24} />
                        </div>
                        <div>
                          <h3
                            className={`font-bold text-xl flex items-center gap-3 ${isDark ? "text-white" : "text-gray-900"}`}
                          >
                            {sub.form_name}
                            {sub.is_duplicate && (
                              <span
                                className={`text-xs px-3 py-1 rounded-full font-bold ${isDark ? "bg-amber-900/50 text-amber-400" : "bg-amber-100 text-amber-700"}`}
                              >
                                درخواست مکرر
                              </span>
                            )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                      {sub.data &&
                        Object.entries(sub.data).map(([key, val]) => (
                          <div
                            key={key}
                            className={`p-4 rounded-2xl ${isDark ? "bg-slate-900/50" : "bg-gray-50"}`}
                          >
                            <span className="text-xs text-gray-400 block mb-1">
                              {key}
                            </span>
                            <span
                              className={`text-sm font-bold ${isDark ? "text-slate-200" : "text-gray-700"}`}
                            >
                              {String(val)}
                            </span>
                          </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid gap-8">
                {forms?.map((form) => (
                  <div
                    key={form.id}
                    className={`p-8 rounded-[2.5rem] border transition-all ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-100 shadow-sm"}`}
                  >
                    ...
                    <div className="space-y-6">
                      <h4 className="font-bold text-sm text-gray-400 uppercase tracking-widest">
                        ساختار فیلدها
                      </h4>
                      {form.fields?.map((field, idx) => (
                        <div
                          key={idx}
                          className={`flex items-center gap-6 p-6 rounded-[2rem] ${isDark ? "bg-slate-900/50" : "bg-gray-50"}`}
                        >
                          <div className="flex-1 grid grid-cols-3 gap-6">
                            ...
                            <div className="flex items-center gap-3 pt-4">
                              ...
                              />
                              <span
                                className={`text-sm font-bold ${isDark ? "text-slate-400" : "text-gray-600"}`}
                              >
                                اجباری
                              </span>
                            </div>
                        ...
                        <button
                          onClick={async () => {
                            try {
                              ...
                              if (res.ok)
                                setNotification({
                                  message: "تغییرات فرم با موفقیت ذخیره شد",
                                  type: "success",
                                });
                              else
                                setNotification({
            ...
            <div className="space-y-8">
              ...
              <div className="grid gap-8">
                {content?.map((block) => (
                  <div
                    key={block.id}
                    className={`p-8 rounded-[2.5rem] border shadow-sm space-y-6 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-100"}`}
                  >
                    ...
                    <div className="space-y-3">
                      <label
                        className={`block text-sm font-bold mr-2 ${isDark ? "text-slate-400" : "text-gray-700"}`}
                      >
                        عنوان بخش
                      </label>
                      <input
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          if (res.ok)
                            setNotification({
                              message: "محتوای بخش ذخیره شد",
                              type: "success",
                            });
                          else
                            setNotification({
                              message: "خطا در ذخیره محتوا",
                              type: "error",
                            });
                        } catch (err) {
                          console.error(err);
          ...
          {activeTab === "settings" && settings && (
            <div className="space-y-8">
              ...
              <div
                className={`p-10 rounded-[3rem] border shadow-sm space-y-10 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-100"}`}
              >
                <div className="grid md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-3 md:col-span-2">
                    <label
                      className={`block text-sm font-bold mr-2 ${isDark ? "text-slate-400" : "text-gray-700"}`}
                    >
                      لینک ویدیو (صفحه اصلی)
                    </label>
                    <input
                      className={`w-full px-6 py-4 rounded-2xl border outline-none transition-all ${isDark ? "bg-slate-900 border-slate-700 text-white focus:ring-blue-500/20" : "bg-white border-gray-200 text-gray-900 focus:ring-blue-500/10"}`}
                      value={settings.video_url}
                      onChange={(e) =>
                        setSettings({ ...settings, video_url: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-3 md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          success_video_url: e.target.value,
                        })
                      }
                    />
                  </div>
                ...
                <div className="space-y-6">
                  <h4
                    className={`font-bold text-lg border-b pb-4 ${isDark ? "text-white border-slate-700" : "text-gray-900 border-gray-100"}`}
                  >
                    شبکه‌های اجتماعی
                  </h4>
                  {settings.social_links?.map((link, idx) => (
                    <div
                      key={idx}
                      className={`flex gap-6 items-end p-6 rounded-[2rem] ${isDark ? "bg-slate-900/50" : "bg-gray-50"}`}
                    >
                      ...
                      <button
                        onClick={() => {
                          const newLinks = (settings.social_links || []).filter(
                            (_, i) => i !== idx,
                          );
                  ...
                  <button
                    onClick={() => {
                      const newLinks = [
                        ...
                        { platform: "جدید", url: "https://", icon: "Link" },
                      ];
                      setSettings({ ...settings, social_links: newLinks });
                    }}
                ...
                <div className="space-y-6">
                  ...
                  {settings.quotes?.map((quote, idx) => (
                    <div key={idx} className="flex gap-4 items-center">
                      ...
                      <button
                        onClick={() => {
                          const newQuotes = (settings.quotes || []).filter(
                            (_, i) => i !== idx,
                          );
                          setSettings({ ...settings, quotes: newQuotes });
                        }}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.ok)
                        setNotification({
                          message: "تمامی تنظیمات با موفقیت ذخیره شدند",
                          type: "success",
                        });
                      else
                        setNotification({
